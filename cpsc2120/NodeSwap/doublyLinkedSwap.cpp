...
void swapWithNext(Node * p) {
    ...
    //Test if the node is the tail
    if (p -> next != nullptr) {
        /*Test if the next node is the tail and if the previous
        node is the head*/
        if (nextNode -> next != nullptr && p -> prev != nullptr) {
            nextNode = p -> next;
            prevNode = p -> prev;
            Node* temp = nextNode -> next;

            //Seven changes to successfully swap node pointers
